...
class CoordinateTransform():
    ...
    def solve_pde(self):
        ...
        l_t = np.zeros(self.nt + 1)
        D_xt = np.zeros((self.nx + 2, self.nt + 1))
        u_xt = np.zeros((self.nx + 2, self.nt + 1))
    ...
    def _update_l(self, l_, u_x):
        return l_ + u_x[-2] * self.dt


    def _update_D(self, l_1, l_2, u_x, D_x, current_t):
        # A, H = c["A"], c["H"]
    ...
    def _update_u_linear_friction(self, l_1, l_2, u_x, h_x, D_x1, D_x2):
    ...
    def _update_u_quadratic_friction(self, l_1, l_2, u_x, h_x, D_x1, D_x2):

        # some different options to compute lambda
