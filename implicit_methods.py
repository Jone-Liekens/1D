...
def update_u3(l__t1, l__t2, u_x_t, x_x, h_x, D_x_t, D_x_t2, dt, dx, c_d, h0, kappa):
    ...
    u_x_guess = 1e100*np.ones(u_x_t.shape) # to make sure there is a large error
    u_x_t2 = u_x_t

    while np.linalg.norm(u_x_t2 - u_x_guess) > 1e-8:
        # print(np.linalg.norm(u_x_t2 - u_x_guess)) # usually converges in 2/3/4 steps 

        u_x_guess = u_x_t2
        Lambda = c_d * np.abs(u_x_guess) / (D_x_t2 + h0)**(4/3)
...
if __name__ == "__main__":
    ...
    plt.legend(["Linear friction", "Quadratic friction n = 0.03", "Quadratic friction n = " + str(n)])
